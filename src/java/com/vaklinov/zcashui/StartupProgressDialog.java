...
public class StartupProgressDialog extends JFrame {
    ...
    public StartupProgressDialog(ZCashClientCaller clientCaller) 
    {
		...
		JLabel zcashWalletLabel = new JLabel(
		    ...
		    "Asofe<span style=\"font-style:italic;font-weight:bold;font-size:1.5em;vertical-align:super\">" + 
...
//        }
        ...
        Log.info("Splash: checking if asofed is already running...");
        ...
        if (!shouldStartZCashd) {
        	Log.info("Splash: asofed already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	Log.info("Splash: asofed will be started...");
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
            	Log.info("Stopping asofed because we started it - now it is alive: " + 
                ...
                try 
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	Log.info("Waiting for " + ((end - start) / 1000) + " seconds for asofed to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                	Log.info("asofed is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                        //System.out.println("asofed is still alive, killing forcefully");
	                        //daemonProcess.destroyForcibly();
	                    } else
	                    	Log.info("asofed shut down successfully");
                } catch (Exception bad) {
                	Log.error("Couldn't stop asofed!", bad);
                }
            }
        });
